...
## 0.1.2-rc.2 - 2026-03-17

- 继续拆平安装、运行时设置、备份与 SQLite 恢复链路，补齐后端事务/回滚编排的模块边界，并清理不再使用的数据库辅助接口。
- 精简安装向导与系统设置页，改成更清晰的 page controller / summary 结构，同时收口导航状态与 API client 的交互细节。
- 补充共享管理侧协议类型、错误映射和恢复状态建模，减少前后端边界上的重复定义与分支散落。
- 保持 RC 发布链路一致性：候选版版本号、changelog、健康状态示例和 runbook 当前候选版口径统一到 `0.1.2-rc.2`。
