...
use crate::config::normalize_mysql_compatible_url;

const DEFAULT_BACKUP_COMMAND_TIMEOUT: Duration = Duration::from_secs(30 * 60);

pub(super) struct MySqlDumpTarget {
...
pub(super) async fn run_dump_command_with_result_file(
    command_name: &str,
    mut command: Command,
    output_path: &Path,
) -> anyhow::Result<ExternalCommandOutcome> {
...
pub(super) fn mysql_dump_binary() -> anyhow::Result<String> {
    override_or_binary("AVENRIXA_MYSQL_DUMP_BIN", &["mysqldump", "mariadb-dump"])
        .ok_or_else(|| anyhow::anyhow!("未找到 mysqldump 或 mariadb-dump"))
}

pub(super) fn parse_mysql_dump_target(database_url: &str) -> anyhow::Result<MySqlDumpTarget> {
...
pub(super) fn mysql_local_ssl_disable_args(_target: &MySqlDumpTarget) -> &'static [&'static str] {
    &[]
}
