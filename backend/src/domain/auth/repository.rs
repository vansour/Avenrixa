...
impl AuthRepository for MySqlAuthRepository {
    ...
    async fn reset_password_by_token(
        ...
        sqlx::query("UPDATE users SET password_hash = ? WHERE id = ?")
            ...
            .execute(&mut *tx)
            .await?;
        sqlx::query("UPDATE password_reset_tokens SET used_at = ? WHERE token_hash = ?")
            .bind(Utc::now())
            .bind(token_hash)
            .execute(&mut *tx)
            .await?;
    ...
    async fn store_email_verification_token(
        ...
        let mut tx = self.pool.begin().await?;
        sqlx::query("DELETE FROM email_verification_tokens WHERE user_id = ? AND used_at IS NULL")
            .bind(user_id)
            .execute(&mut *tx)
            .await?;
        sqlx::query(
    ...
    async fn verify_email_by_token(
        ...
        sqlx::query("UPDATE users SET email_verified_at = ? WHERE id = ?")
            ...
            .execute(&mut *tx)
            .await?;
        sqlx::query("UPDATE email_verification_tokens SET used_at = ? WHERE token_hash = ?")
            .bind(Utc::now())
            .bind(token_hash)
            .execute(&mut *tx)
            .await?;
...
impl AuthRepository for DatabaseAuthRepository {
    async fn find_user_by_id(&self, id: Uuid) -> Result<Option<User>, sqlx::Error> {
        match self {
            ...
            Self::MySql(repo) => repo.find_user_by_id(id).await,
    ...
    async fn find_user_by_email(&self, email: &str) -> Result<Option<User>, sqlx::Error> {
        match self {
            ...
            Self::MySql(repo) => repo.find_user_by_email(email).await,
    ...
    async fn create_user(&self, user: &User) -> Result<(), sqlx::Error> {
        match self {
            ...
            Self::MySql(repo) => repo.create_user(user).await,
    ...
    async fn update_user_password(
        ...
        match self {
            ...
            Self::MySql(repo) => repo.update_user_password(user_id, password_hash).await,
    ...
    async fn verify_email_by_token(
        ...
        match self {
            ...
            Self::MySql(repo) => repo.verify_email_by_token(token_hash).await,
