...
const REVOKED_TOKENS_INSERT_SQL: &str = r#"INSERT INTO revoked_tokens (token_hash, expires_at)
             VALUES ($1, $2)
             ON CONFLICT (token_hash) DO UPDATE
             SET expires_at = EXCLUDED.expires_at"#;
const REVOKED_TOKENS_EXISTS_SQL: &str =
    "SELECT 1 FROM revoked_tokens WHERE token_hash = $1 AND expires_at > NOW()";
const REVOKED_TOKENS_PURGE_SQL: &str = "DELETE FROM revoked_tokens WHERE expires_at < $1";
...
impl AuthStateRepository for PostgresAuthStateRepository {
    ...
    async fn revoke_token_hash(
        ...
        sqlx::query(REVOKED_TOKENS_INSERT_SQL)
            .bind(token_hash)
            .bind(expires_at)
            .execute(&self.pool)
            .await?;
    ...
    async fn is_token_hash_revoked(&self, token_hash: &str) -> Result<bool, sqlx::Error> {
        let result = sqlx::query_scalar::<_, i32>(REVOKED_TOKENS_EXISTS_SQL)
            .bind(token_hash)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn purge_expired_revoked_tokens(
        ...
        let result = sqlx::query(REVOKED_TOKENS_PURGE_SQL)
            .bind(cutoff)
            .execute(&self.pool)
            .await?;
