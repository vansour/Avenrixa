...
use self::mysql::{
    dump_current_mysql_database_for_rollback, load_runtime_settings_from_mysql,
    looks_like_mysql_dump, restore_mysql_dump_into_current_database,
};
use self::sqlite_ops::{
    create_sqlite_snapshot, inspect_sqlite_backup, load_runtime_settings_from_path,
    replace_database_file,
};
use self::state::{
    append_object_rollback_anchor_warnings, backup_database_kind_from_filename,
    backup_database_kind_from_pending, backup_file_summary, backup_path,
    clear_pending_restore_plan, load_last_restore_result, load_pending_restore_plan,
    persist_last_restore_result, persist_pending_restore_plan, restore_database_label,
    storage_summary_from_snapshot, unknown_storage_summary,
};
use crate::audit::log_audit_db;
use crate::backup_manifest::{backup_directory, load_backup_manifest, storage_signature};
use crate::bootstrap::resolve_sqlite_database_path;
use crate::config::{Config, DatabaseKind};
use crate::db::ADMIN_USER_ID;
#[cfg(test)]
use crate::db::{DatabasePool, INSTALL_STATE_SETTING_KEY};
use crate::models::{
    BackupDatabaseFamily, BackupRestorePrecheckResponse, BackupRestoreResult,
    BackupRestoreScheduleResponse, BackupRestoreStatus, BackupRestoreStatusResponse,
    PendingBackupRestore, backup_database_family_from_config,
};
...
pub async fn schedule_restore(
    ...
    persist_pending_restore_plan(&pending)
        .await
        .map_err(crate::error::AppError::Internal)?;
...
mod tests {
    ...
    use crate::runtime_settings::{
        RuntimeSettings, SETTING_LOCAL_STORAGE_PATH, SETTING_STORAGE_BACKEND,
    };
