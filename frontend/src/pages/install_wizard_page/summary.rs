...
impl InstallWizardStep {
    ...
    pub(super) fn title(self) -> &'static str {
        match self {
            Self::Admin => "第 1 步：管理员设置",
            Self::General => "第 2 步：站点配置",
            Self::Storage => "第 3 步：存储设置",
            Self::Review => "第 4 步：确认",
        }
    }
}
...
pub(super) fn install_storage_ready(form: SettingsFormState) -> bool {
    !(form.local_storage_path)().trim().is_empty()
}

pub(super) fn current_install_storage_summary(_form: SettingsFormState) -> String {
    "本地存储".to_string()
}
